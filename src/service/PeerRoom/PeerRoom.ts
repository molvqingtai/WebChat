...
export default class PeerRoom<Message extends object> extends EventHub {
  readonly peer: Peer
  ...
  sendMessage(message: Message, id?: string | string[]) {
    if (!this.room) {
      this.once('action', () => {
        if (!this.room) {
    ...
    if (!this.room) {
      this.once('action', () => {
        if (!this.room) {
          ...
          this.room.on('message', (message) => callback(JSONR.parse(message) as Message))
        }
      })
    } else {
      this.room.on('message', (message) => callback(JSONR.parse(message) as Message))
