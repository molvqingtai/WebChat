...
describe('reconnect action availability', () => {
  ...
  it('reprojects resize locally and persists only user drag coordinates', () => {
    const value = source()
    const layout = readFileSync(path.resolve(process.cwd(), 'src/app/content/views/app-layout/index.tsx'), 'utf8')

    expect(layout).toContain('const viewport = useWindowResize()')
    expect(layout).toContain('const geometry = getAppGeometry(appPosition, viewport, appOpen)')
    expect(layout).toContain('captureAppButtonPosition(position, viewport, appOpen)')
    expect(layout).toContain('x: geometry.point.x')
    expect(layout).toContain('y: geometry.point.y')
    expect(layout).toContain('...geometry.bounds')
    expect(layout).toContain('onChange: handlePositionChange')
    expect(layout).toContain('<AppMain open={appOpen} geometry={geometry.shell}>')
    expect(layout).toContain('launcherSize={geometry.launcher.size}')
