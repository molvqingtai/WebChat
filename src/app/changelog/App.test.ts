...
describe('ChangelogView', () => {
  it('renders the local release record and exact outbound commands', () => {
    ...
    expect([...header.matchAll(/<img\b[^>]*?\ssrc="([^"]+)"/g)].map(([, src]) => src)).toEqual(['/logo.png'])
    expect(header).not.toMatch(/<img\b[^>]*?\ssrc="https?:\/\//i)
    expect(header).not.toMatch(/shields\.io/i)
    const logoMatch = /<img\b[^>]*?\ssrc="\/logo\.png"(?:\s[^>]*)?>/.exec(header)
    const headingIndex = header.indexOf('<h1')
    const badgeIndex = header.indexOf('data-slot="badge"')
    expect(logoMatch).not.toBeNull()
    expect(headingIndex).toBeGreaterThanOrEqual(0)
    expect(badgeIndex).toBeGreaterThanOrEqual(0)
    expect(logoMatch!.index).toBeLessThan(headingIndex)
