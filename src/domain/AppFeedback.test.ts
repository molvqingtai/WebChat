...
describe('application feedback ownership', () => {
  ...
  it('composes real lease + readiness + feedback: departure never creates loading and restore reconciles ready', async () => {
    const domain = 'https://example.test'
    const pageId = 'page-a'
    ...
    // Departure: the composition owner silences feedback first, then the real lease detach changes
    // page-local host phase to none -> connecting. Silenced feedback must NOT create a loading entry.
    fixture.store.send(feedback.command.SilenceFeedbackCommand())
    lease.detach()
    await flushMicrotasks()
    expect(fixture.toast.loading).not.toHaveBeenCalled()

    // BFCache restore: the same lease re-inits to ready and feedback resumes, dismissing without a
    // success Toast (the visible page converges to ready, not stuck on loading).
    fixture.store.send(feedback.command.ResumeFeedbackCommand())
    await lease.init()
    await flushMicrotasks()
    expect(fixture.toast.loading).not.toHaveBeenCalled()
