      ...
      try {
        ...
        try {
          ...
          if (writable && transaction.mutated.size > 0) {
            // functional-loop: owner-commit — ordered per-store state commit during live iteration
            for (const store of transaction.mutated) {
              this.state.stores.set(store, working.get(store) ?? new Map())
            }
            // functional-loop: owner-commit — ordered per-instance notification with no bulk primitive
