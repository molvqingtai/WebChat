...
- `pnpm lint` applies ESLint + Prettier autofixes; `pnpm check` runs the TypeScript compiler in no-emit mode for API regressions.

## Coding Style & Naming Conventions
- Follow Prettier defaults (`semi: false`, `singleQuote: true`, `printWidth: 120`); never mix manual formatting with tool output.
...
- Tailwind CSS utilities power styling; prefer component-level class composition via `clsx` as configured in `.prettierrc`.
- Run `pnpm lint` before committing to keep the ESLint cache current and avoid husky rejections.
