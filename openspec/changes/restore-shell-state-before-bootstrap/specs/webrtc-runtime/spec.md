...
### Requirement: Persisted shell state restores independently of application bootstrap
...
Pending, rejected, timed-out, canceled, or unavailable application bootstrap SHALL NOT skip the shell-state read, clear the saved choice, overwrite it with the default, or prevent a new shell choice from being persisted. Bootstrap outcome MAY gate only the capabilities that depend on it. Restoring a saved expanded state SHALL NOT count as automatic opening; when no saved state exists, the existing default collapsed state SHALL remain authoritative.

The document shell SHALL own exactly one current hydration and persistence lifecycle. Retry, dependency recovery, and ready application activation SHALL reuse that same shell state and SHALL NOT create another storage read owner, persistence watcher, state mirror, root, or shell effect. Existing storage key, record shape, version, and the semantics of other stored status fields SHALL remain unchanged.

A user expansion or collapse accepted before hydration settles SHALL be newer than the pending stored snapshot. The accepted choice SHALL remain visible, SHALL be persisted through the shell-owned lifecycle, and SHALL NOT be overwritten by late hydration. A result from a superseded shell or document generation SHALL NOT mutate or repersist the current shell. Bootstrap readiness, failure, Retry, and late settlement SHALL never independently change expanded/collapsed state.

#### Scenario: Persisted expanded state restores while bootstrap is pending

- **GIVEN** the current document has mounted its one shell, local shell state records expanded, and an application bootstrap dependency remains pending
...
#### Scenario: Persisted expanded state survives every bootstrap terminal

- **GIVEN** local shell state records expanded and the current shell has begun hydration
- **WHEN** browser-sync, page-local configuration, IndexedDB, or Runtime bootstrap rejects, times out, is canceled, or settles unavailable
- **THEN** the same shell SHALL restore or retain expanded state and SHALL NOT write collapsed merely because bootstrap failed

#### Scenario: Persisted collapsed state remains collapsed

- **GIVEN** local shell state records collapsed
- **WHEN** the shell hydrates while bootstrap is pending, succeeds, fails, times out, or later recovers
- **THEN** the shell SHALL remain collapsed until a user expands it, and neither restoration nor bootstrap transition SHALL automatically open it
...
- **THEN** the existing default collapsed state SHALL remain in effect without changing bootstrap operation, feedback, or Refresh eligibility
...
- **GIVEN** the shell has started or completed its one hydration and application bootstrap has failed
- **WHEN** the user invokes Refresh and the next bootstrap generation succeeds or fails
...
The existing AppButton actions menu SHALL remain reachable from the mounted shell before full application readiness and SHALL retain one familiar Refresh slot. Before readiness, that slot SHALL represent only the current whole-bootstrap operation as specified by `Refresh control projects current connection loading`; unrelated menu actions MAY retain their existing individual dependency eligibility. After readiness, the same slot SHALL return atomically to the existing current-domain ChatRoom contract described below. No second bootstrap Retry button or ready-state reconnect control SHALL be added.
...
The application SHALL expose one generic Toast feedback capability, not a reconnect-owned or bootstrap-owned presenter. Bootstrap terminal errors, reconnect, join retry, Runtime readiness, and unrelated application notifications SHALL publish through the existing generic Toast descriptor/presentation APIs and share one shell-lifetime Toaster. `src/app/content/components/reconnect-toast.tsx`, `useReconnectToast`, reconnect-specific or bootstrap-specific presenter/renderer naming, and any independent source-local Toast lifecycle truth SHALL remain absent. Any presentation adapter SHALL consume only generic Toast descriptors, stable IDs, and presentation acknowledgements; it SHALL NOT directly import or own bootstrap, ChatRoom, Readiness, Runtime, network, or panel behavior. Business lifecycle effects MAY map source events to generic descriptors, but the matching bootstrap or ready application request SHALL remain the sole owner of operation outcome, duplicate rejection, button pending state, and stale-generation/request fencing. Generic Toast state SHALL NOT become a second bootstrap, recovery pending, or network truth.

The one Toaster SHALL be owned by the persistent shell rather than the ready-only `AppMain`, so bootstrap failure feedback is available before application activation and regardless of panel expanded/collapsed state. It SHALL remain the only renderer and retain `richColors`, current `themeMode`, `offset="70px"`, `visibleToasts={1}`, `position="top-center"`, and `dark:bg-slate-950 border dark:border-slate-600` Toast classes. There SHALL be no bootstrap-specific or reconnect-specific Toaster, second renderer, host-page error page, source-specific restyling, or custom geometry, width, content-fit, placement, pointer, opacity-tracking, pseudo-element, or eligibility styles. Neither bootstrap success, normal Runtime ready, nor successful manual Refresh SHALL publish a success descriptor. Cleanup SHALL address only a matching current ID, SHALL NOT use unscoped/global dismissal, and SHALL preserve unrelated Toasts.

The mounted pre-ready panel SHALL retain its existing `Preparing WebChat` loading content while the current initial or user-retried bootstrap operation is active. A current, non-superseded bootstrap rejection, deadline, or unavailable result SHALL leave the shell, launcher, actions menu, and current expanded/collapsed state intact; end that attempt's busy/control loading; publish one normalized generic error descriptor; and expose no panel-local `WebChat unavailable`, alert/error/result view, or Retry button. Superseded-generation and unmount cancellation SHALL publish no user error. The generic error SHALL use the existing default lifetime and existing user-dismissal/replacement rules. Ordinary Retry, ready settlement, or panel state change SHALL NOT actively dismiss the current error merely because business state changed. Raw exceptions remain diagnostic rather than user-facing copy.

Runtime lifecycle SHALL retain its immediate-replay `connecting | ready | unavailable` state for recovery and send preconditions after the application is ready. ReadinessDomain SHALL remain the sole ready-application readiness-transition authority. Its State-setting Command SHALL compare every mapped extern input to current State: equal input SHALL perform no State write and emit no `StateChangedEvent`, while a different input SHALL update State and emit exactly one Event. The current Query SHALL remain immediately available independently of transition Events. The pre-ready bootstrap attempt owner SHALL remain separate from Runtime readiness and SHALL NOT add a duplicate readiness State to ClientLease, the Readiness implementation adapter, AppFeedback, or Toast presentation.
...
- **THEN** the same Refresh slot SHALL be present with bootstrap-context eligibility and an accessible setup-retry label, while no separate panel Retry control SHALL exist and no ChatRoom reconnect SHALL be dispatched
...
- **THEN** Refresh SHALL be visibly disabled with an accessible non-actionable label and SHALL dispatch no ChatRoom recovery request; this ready-context rule SHALL NOT prevent pre-ready bootstrap Retry
...
- **GIVEN** the ready application has attached its feedback sources to the shell-level generic Toaster
...
- **GIVEN** the shell-level Toaster remains mounted and the join or leave/join ports settle before the request-owned loading Toast receives its first visible paint
...
#### Scenario: One shell-level Toaster preserves generic visuals

- **WHEN** the shell renders before or after ready application activation
- **THEN** it SHALL contain exactly one generic Toaster with `richColors`, current theme, `offset="70px"`, `visibleToasts={1}`, `position="top-center"`, and the existing dark Toast classes, without a second bootstrap/reconnect renderer or custom geometry/pointer styling

#### Scenario: Generic Toast surface carries independent business sources

- **GIVEN** unrelated application Toast feedback exists before or during bootstrap recovery, ready-context recovery, or a Runtime readiness transition
...
#### Scenario: Bootstrap failure uses Toast instead of panel error

- **GIVEN** the shell and generic Toaster are mounted and a browser-sync, page-local configuration, IndexedDB, or Runtime bootstrap stage is current
...
#### Scenario: Loading shell is the only pre-ready panel state

- **GIVEN** application bootstrap has not reached ready
- **WHEN** the current initial or retried attempt is active or has terminally failed
- **THEN** the open panel MAY retain the existing `Preparing WebChat` shell content, its busy semantics SHALL reflect only an active attempt, and it SHALL render no independent unavailable/error/result view or Retry action; terminal feedback SHALL use the generic Toast and recovery SHALL use actions-menu Refresh

#### Scenario: Closed panel retains Toast and operation ownership

- **GIVEN** the main panel is collapsed and the shell-level generic Toaster remains mounted
- **WHEN** bootstrap fails or an enabled user activates bootstrap or ready-context Refresh
...
- **GIVEN** the shell-level Toaster is mounted for a ready-context recovery request
...
- **GIVEN** bootstrap or ready-context recovery has already terminated and its generic Toast has been dismissed, expired, or replaced
...
- **WHEN** the main plugin panel expands or collapses during bootstrap or ready-context recovery
- **THEN** the same operation, matching Refresh projection, and shell-level Toast lifecycle SHALL continue without cancellation, replay, restart, duplication, remount, or panel mutation
...
The existing AppButton Refresh control SHALL project exactly one current operation owner for its lifecycle context. Before full application readiness, the current initial or user-retried bootstrap attempt SHALL own the control: while that attempt is active, Refresh SHALL be present in the existing actions menu, disabled, and continuously rotating; after a matching terminal failure, it SHALL become enabled and static so the user can retry the whole bootstrap; and after bootstrap success, its authority SHALL end atomically as the ready application activates. A pre-ready Refresh SHALL NOT require configured user identity and SHALL NOT dispatch ChatRoom/WorldRoom reconnect directly.
...
One accepted pre-ready activation SHALL start one fresh bounded bootstrap generation in the same root/store, return the open panel to active `Preparing WebChat` busy semantics, and fence every late result from the failed or superseded generation. Matching success SHALL activate the application in place, switch Refresh to ready-context semantics, and publish no success Toast. A current, non-superseded matching failure SHALL end rotation, restore bootstrap Retry eligibility, and publish one normalized generic error through the shell-level Toaster without adding a panel error/Retry view. Superseded-generation and unmount cancellation SHALL remain silent. The error SHALL retain its existing default presentation lifetime; business readiness or Retry SHALL NOT actively dismiss it absent an explicit successor descriptor.

When a ready-context logical operation reaches ready, logically final failure, cancellation, or another defined terminal outcome, the fixed owner's Toast entry SHALL leave `loading` and its control loading SHALL end in the same transition. Success dismisses only current matching loading. Only a logically final connection failure replaces it with exact `Connection failed` and does not actively hide that error; an intermediate attempt, cancellation with continuation, retry/handoff, or obsolete completion SHALL not end or replace current loading. The icon SHALL stop and ordinary Refresh eligibility SHALL be recomputed atomically. Settlement from an expired, detached, aborted, stale-deadline, or superseded bootstrap or ready generation SHALL not stop the icon, enable the button, change context, or clear/replace feedback while a newer or continuing logical operation remains current. The control SHALL add no second loading owner, timer, connection truth, or browser-specific behavior.

#### Scenario: Initial bootstrap projects disabled rotation

- **GIVEN** the shell and actions-menu Refresh are mounted before ready
- **WHEN** the current initial bootstrap attempt is active
- **THEN** Refresh SHALL be disabled and rotate continuously, another activation SHALL start nothing, and the open panel SHALL expose `Preparing WebChat` without a bootstrap error/Retry view

#### Scenario: Bootstrap failure restores the same Refresh

- **GIVEN** a current pre-ready bootstrap attempt owns disabled rotating Refresh
- **WHEN** that attempt rejects, times out, or settles unavailable while it remains current
- **THEN** only the matching rotation SHALL stop, the same Refresh SHALL become eligible for whole-bootstrap Retry, one generic error Toast SHALL present, and the panel SHALL gain no second Retry control

#### Scenario: Pre-ready Refresh owns one fresh bootstrap generation

- **GIVEN** bootstrap is terminally unavailable and the existing Refresh action is enabled
- **WHEN** the user activates it
- **THEN** exactly one fresh bounded bootstrap generation SHALL start in the same root/store, Refresh SHALL disable and rotate, the loading shell SHALL become busy, and no ChatRoom or WorldRoom reconnect command SHALL dispatch

#### Scenario: Bootstrap success switches Refresh context without replay

- **GIVEN** a current bootstrap generation owns Refresh and every required dependency becomes ready
- **WHEN** the ready application activates in the same shell
- **THEN** bootstrap ownership SHALL terminate once, Refresh SHALL switch to existing ready-context eligibility without a success Toast, and late bootstrap results SHALL NOT regain control or dispatch ready operations
...
- **GIVEN** one bootstrap or ready loading owner was superseded and a newer current owner is active
