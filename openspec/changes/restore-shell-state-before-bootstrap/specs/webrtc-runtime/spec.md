...
The App/AppMain tree SHALL keep `Header`, `Main`, `Footer`, conditional `Setup`, and the generic Toaster in one composition throughout initialization and SHALL render no independent loading, busy, unavailable, error, result, alert, or Retry status component. A current initial or user-retried initialization operation SHALL issue one `Preparing WebChat` loading command through `Toast.ts` under the stable initialization Toast ID. A genuine current-page, non-superseded rejection, deadline, or unavailable result SHALL leave the page, launcher, actions menu, and current expanded/collapsed state intact; end only that attempt's control/loading ownership; and issue one same-ID error command whose copy is exactly the original `error.message` as a direct replacement for loading, without a preceding cancel, prefix, suffix, wrapper, mapping, normalization, or replacement copy. A genuine failure with no current affected page/live route or no user impact SHALL call `console.error(error)` directly and SHALL NOT manufacture a Toast destination. Superseded-generation and unmount cancellation SHALL publish no user error. The generic error SHALL use the default lifetime and user-dismissal/replacement rules. Ordinary Retry, ready settlement, or panel state change SHALL NOT actively dismiss the current error merely because business state changed. Structured routing fields and other diagnostic context SHALL remain internal and SHALL NOT decorate the original user-facing message.
...
- **THEN** the application SHALL record the original request-local error on that request, return join status to initial, settle only matching button/Toast feedback, and re-enable Refresh after the request terminal without starting an automatic loop
...
- **THEN** the panel SHALL contain no `WebChat unavailable`, alert/error/result page, or Retry button; one same-ID generic error Toast containing exactly the original `error.message` SHALL present for its existing default lifetime without a preceding cancel or decorated/replacement copy; the shell state SHALL remain unchanged; and the actions-menu Refresh SHALL become retryable
...
- **THEN** the App/AppMain tree SHALL keep its normal composition without any independent loading, busy, unavailable, error, result, or Retry status component; an active attempt SHALL use one matching `Preparing WebChat` command through `Toast.ts`, a genuine current-page terminal failure SHALL directly replace it with a same-ID error containing exactly the original `error.message`, and recovery SHALL use actions-menu Refresh
...
- **THEN** only the matching rotation SHALL stop, the same Refresh SHALL become eligible for whole-initialization Retry, one matching generic error Toast containing exactly the original `error.message` SHALL directly replace loading without a preceding cancel, and the panel SHALL gain no status or second Retry control
