> **Draft coding status (2026-08-19):** Dual-provider source exact `1080dece375c576dd72304d6ed3ebfd9418b8487` passed hosted CI `32217786680` and fresh cumulative CODE FINAL PASS `0/0/0`. It uses immutable fork exact `0deb0f0fa2fc1ec17f05043f8fb47bc4c11de71f` only for Draft verification. The official upstream `@rtco/client` replacement, regenerated lockfile, complete gates, and fresh coding review remain mandatory before `develop` merge.
...
- [x] 1.6 Freeze immutable fork use to Draft implementation/verification and require an official upstream client release before any `develop` merge.
...
- [x] 4.1 Run the root shared RoomTransport contract unchanged against Artico and Trystero, covering identity, lifecycle, trusted source, omitted/string/array/empty targets, mixed readiness, zero recipients, provider events/failures, and dispose.
...
- [x] 4.3 Prove Artico provider-specific ownership/signaling/recovery/readiness behavior and Trystero provider-specific Nostr/lifecycle/error-silence behavior without cross-provider leakage; structurally require contextual `RoomTransport*` filenames and reject redundant provider-prefixed names.
- [x] 4.4 Prove all room-wide producers remain omitted-target, all History/catch-up sends retain exact targets, skipped peers receive no late replay, and existing queue/generation/owner/no-retry boundaries do not regress.
...
- [ ] 5.3 Update canonical task/status truth after the official-package exact has coding review and exact CI, then clear the four-item batch gate before the ordinary protected merge to `develop`; no separate Owner acceptance is required.
