...
A dependency/export/residue scan SHALL prove that `src/protocol/**` has no reverse imports or local/UI/internal Runtime exports; Chat/UI do not import Runtime/provider/Database primitives or concrete adapters; MessageStore is not an extern/public export; Artico imports are provider/composition-only; and no removed Network owner, WireExtern route, ChatRoom cache, false Module name, status/outbox/retry, or legacy record alias remains. No agent SHALL treat `pnpm dev` or ad hoc browsing as release evidence. After Reviewer logic PASS, QA SHALL independently use exact-bound production Chrome MV3 and Firefox MV2 artifacts with isolated profiles and process/profile cleanup while the same immutable exact is synchronized to the Owner's local test environment for independent product acceptance. QA reconnect evidence SHALL record the viewport, panel placement, computed panel transform, panel rectangle, every request-owned loading/error Toast rectangle and its mounted/visible/removed/computed-opacity state, computed pointer behavior for the item and its `::before`/`::after`, and actual `document.elementFromPoint()` plus real-click results for an ordinary control and eligible Toast item/action/close. The real-browser matrix SHALL cover pre-mount, steady, expanded/swiping, success retirement, loading-to-error retirement, and error retirement across left, right, and narrow/mobile placement. JSX nesting, an isolated Toaster mount, direct synthetic `.click()`, or Toast containment without hit-testing SHALL NOT substitute for those real rendered controls. QA automation MAY support but SHALL NOT replace the Owner's final product verdict. Commit, push, and PR update need no separate Owner authorization; PR merge and `master` release SHALL remain separate explicit Owner authorizations.
