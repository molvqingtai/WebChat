...
- [x] 1.5 Define backdrop/control/Escape/shell-collapse close, focus restoration, one-image replacement, and progressive View Transition behavior with immediate reduced-motion and unsupported-API fallback.
- [x] 1.6 Exclude hate, reaction, protocol, persistence, host-scroll, upload, download, carousel, speculative media, second-root, second-owner, and runtime image-measurement changes.
...
- [ ] 4.5 Provide the pushed, directly checkout-able branch and exact to the Owner in the parent channel for acceptance, then update final OpenSpec/task truth and keep Ready/merge conditional on the closeout exact's identity and CI.
