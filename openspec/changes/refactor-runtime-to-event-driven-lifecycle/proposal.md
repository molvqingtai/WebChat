...
- Restore World state from committed logical recovery facts rather than raw transport membership counts: validate every retained recovery source by exact `sourcePeerId + sourceGeneration`, keep duplicate active-peer joins idempotent, and advance the generation only after an explicit leave and same-ID rejoin.
