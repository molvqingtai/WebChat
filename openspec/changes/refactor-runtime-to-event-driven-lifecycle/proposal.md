...
- Project current physical World membership separately from committed logical recovery payload: validate every remembered recovery source by exact `sourcePeerId + sourceGeneration`, do not infer missing logical facts from the physical member count, keep duplicate active-peer joins idempotent, and advance the generation only after an explicit leave and same-ID rejoin.
...
- Preserved boundaries: peer wire bytes and schemas, the eight-method application-facing ChatRoom port, product-visible Chat/World behavior, origin-owned durable messages, exact source identity, browser permissions, and selected transport provider semantics. Physical peer observation remains transport state and does not create a logical recovery obligation.
