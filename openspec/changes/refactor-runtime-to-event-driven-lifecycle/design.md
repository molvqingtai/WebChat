...
World transport membership and logical recovery are related but non-equivalent facts. The surviving Offscreen host records a physical peer as soon as the provider reports its join, while the remembered owner-confirmed checkpoint may contain fewer recovery sources than the physical map. Rebind validates every checkpoint entry against the current physical `sourcePeerId + sourceGeneration`; it neither requires checkpoint and physical cardinality to match nor infers a World-local recovery obligation from an adjacent ROOM recovery requirement. Missing recovery fails closed only for an explicit obligation in the same recovery domain.

The returned `TransportProjection.worldRecovery.members` is rebuilt from the complete current physical World map, so an observed peer remains visible even when it has no committed logical payload. Checkpoint-backed remote presences are retained only while their exact `sourcePeerId + sourceGeneration` is current. Local registrations come only from an owner-confirmed local checkpoint bound to the committed local peer and current physical World Room handle. Rebind therefore returns current physical members without inventing presence, local registration, or other committed World facts for an uncommitted peer.

A repeated provider join for a peer that is still active is idempotent and retains its current World source generation, so already-committed recovery remains current. An explicit leave removes that active membership. A later join with the same peer ID creates the next generation and invalidates recovery bound to the old generation. This distinguishes duplicate observation from a real physical incarnation change without weakening stale-source rejection.
...
Validate the complete event/state and browser restart matrix, including Background fresh + Offscreen surviving, Background surviving + Offscreen fresh, both fresh, and both current. Prove Page initial load/rebind, immediate full-before-delta ordering, target-only readiness, single action execution, stale event rejection, recovery-entry validation without physical/logical count equivalence, duplicate active-peer join idempotence, explicit leave/rejoin generation advancement, live-domain non-blocking recovery, zero-page successor fencing, Firefox no-Offscreen behavior, Page no-polling, and absence of reachable legacy lifecycle or compatibility paths. Run strict OpenSpec and the repository's normal source, browser, and delivery gates on one immutable replacement exact before review.
