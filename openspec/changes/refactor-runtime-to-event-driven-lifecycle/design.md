...
World transport membership and logical recovery are related but non-equivalent facts. The surviving Offscreen host records a physical peer as soon as the provider reports its join, while `worldRecovery.members` contains only owner-confirmed, committed logical recovery sources. Rebind therefore validates every recovery entry against the current physical `sourcePeerId + sourceGeneration`; it neither requires the two collections to have equal cardinality nor infers a World-local recovery obligation from an adjacent ROOM recovery requirement. Missing recovery fails closed only for an explicit obligation in the same recovery domain.
