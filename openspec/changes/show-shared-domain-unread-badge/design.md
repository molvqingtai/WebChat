...
`AppStatusDomain` remains the single business owner for shell status and unread attention. The final model has two distinct scopes inside that owner: expanded/collapsed presentation belongs to one physical tab, while unread attention belongs to the WebChat domain shared by its tabs. The existing first-delivery boundary remains responsible for admitting a remote text once, and the existing origin-local synchronization boundary distributes the resulting domain attention.
...
- Preserve one AppStatus business owner while separating tab-local shell presentation from domain-shared unread attention.
...
### 1. Keep one owner with two explicit scopes

`AppStatusDomain` remains the only business owner. Each physical tab owns its expanded/collapsed shell state, so tab A may remain expanded while same-domain tabs B and C remain collapsed. A same-tab reload keeps that tab's shell state without importing a sibling tab's shell state. The domain owns one shared unread-attention truth observed by all of its tabs. Initialization phase, Retry, position, and other existing AppStatus responsibilities remain outside the unread decision.

This separation is semantic as well as physical: an unrelated tab-local shell or position update cannot overwrite a newer domain unread mark or clear, and a domain unread update cannot toggle any tab's shell.

### 2. Mark attention once at the admitted delivery boundary
...
### 3. Read through a user-driven expansion transition
...
### 4. Preserve one exact AppButton indicator
...
- [Concurrent tabs can write different status facts] -> Tab-local shell updates and domain unread updates have independent write semantics, so neither can clobber a newer value in the other scope.
