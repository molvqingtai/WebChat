...
- [x] 1.3 Define the collapsed persistent unread author, immediate later-author replacement on an observing surface, reopen clear, transient-collapse clear, resumed same-domain convergence without lifetime restart, and cross-domain isolation.
- [x] 1.4 Define avatar/image fallback, the optional launcher-scoped View Transition boundary, and the unchanged daily logo, unread badge, launcher geometry, interaction, notification, delivery, protocol, and dependency boundaries.
...
- [ ] 2.2 Reuse the existing first-delivered remote-text event, author identity, field-scoped synchronization, and persistence boundary so a resumed surface converges to the latest accepted open, unread, author, and deadline facts without delivery owning open or restarting an expired lifetime.
...
- [ ] 2.4 Add focused deterministic regressions for exact expiry, same/different-author bursts, stale timeout, expanded/collapsed transitions, reopen, hydration/remount, delayed-surface same-domain convergence, cross-domain isolation, eligibility exclusions, fallback, badge coexistence, optional View Transition scope/failure/supersession, and unchanged launcher behavior.
