...
- Permit `for`, `for...of` (including `for await...of`), and `for...in` only when the loop body genuinely needs `break`, `continue`, or an early return that cannot be expressed directly with `some`, `every`, `find`, `findIndex`, or another result-oriented operation.
...
- Refactor side-effect traversal into an immutable owner-specific plan followed by one explicit owner-level commit invocation. Preserve current ordering, sequential async behavior, short-circuiting, error propagation, resource cleanup, and product behavior.
...
- Add repository-owned structural enforcement as a read-only functional pass of the existing Oxlint quality surface, with focused rule fixtures and no new linter or parser dependency. Retained control-flow loops require a narrow, auditable statement annotation; broad path, file, or rule disables are forbidden.
