...
- [x] 1.3 Freeze all `for` variants under one behavior-equivalence exception, condition-driven `while`/`do...while`, externally side-effect-free result callbacks, the narrow consumed `map` allowance for one indivisible synchronous mixed effect-and-result owner call, explicit synchronous/concurrent action forms, and the fresh exclusive reducer-accumulator allowance.
