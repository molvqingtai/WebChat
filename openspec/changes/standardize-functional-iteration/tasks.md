...
- [x] 1.5 Freeze the existing-toolchain-only, no-new-test, generated-file, Draft-only, and no-delivery boundaries; all source remains behavior-neutral except the exact Owner-approved Artico and History corrections.
- [ ] 1.6 Freeze optional-target delegation, message-intent classification, the fixed-provider prerequisite, one broadcast History request with per-provider loading settlement and arrival-time-independent valid-response merge, targeted state catch-up/responses, and the UI-only manual/full-settlement/ten-second loading-close behavior.
...
- [ ] 2.4 Record the published fixed `@rtco/client` exact and verify that its broadcast/selected-array paths attempt later peers after a stale or closing peer failure, then rethrow the first synchronous room-call error unchanged, before editing the WebChat adapter.
...
- [ ] 3.6 Add no plugin, parser, linter, new package, scanner, source enforcement module, test case, assertion, test abstraction, fixture, mock capability, or coverage expansion; allow only the published fixed `@rtco/client` version, lock resolution, and minimum existing-expectation synchronization.
- [ ] 3.7 Replace adapter recipient construction and local error aggregation with one `room.send(payload, to)` call; broadcast ordinary Chat, normal Session/World publications, and one History request, while preserving targeted Session/World current-state catch-up and targeted History responses.
- [ ] 3.8 Replace per-peer History request fan-out with one request-start loading-settlement snapshot, one request identity whose paginated inventory-request pages each use one no-target broadcast, independent `(syncId, sourcePeerId)` pagination validation, deduplicated all-provider merge regardless of arrival time/connectivity/generation, and a UI-only full-settlement-versus-absolute-ten-second loading race.
...
- [ ] 4.2 Confirm evaluation order, iteration order, call count, sync/async concurrency, return/error behavior, object identity, mutation visibility, event/timer order, DOM, storage/database, wire payloads, persistence, generated output, and public interfaces are unchanged outside the exact Artico/History correction.
...
- [ ] 4.4 Confirm `undefined|string|string[]|[]` target semantics, provider attempt-all plus unchanged first-error propagation, broadcast/target call-site classification, all-provider History merge regardless of response arrival time/connectivity/generation, and UI-only manual dismissal/ten-second timeout match the reviewed authority exactly.
...
- [ ] 5.2 Inspect the exact diff against the 304-file manifest and prove the sole generated exclusion, original `assembleURL`, sole fixed-provider dependency delta, tests' behavioral evidence, wire payloads, persistence, and every non-authorized product behavior remain unchanged.
