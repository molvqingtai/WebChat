...
- Prohibit result-producing callbacks from performing external side effects: they do not mutate inputs, outer bindings, shared or externally reachable objects, or perform I/O, DOM, storage, event, wire, persistence, or other external commits. The only callback effect carriers are the explicit synchronous `forEach` and fully consumed concurrent `Promise.all(...map(...))` forms above.
