...
- Allow a consumed `map` result for a synchronous owner API when no behavior-equivalent bulk operation exists and the API cannot be split behavior-equivalently into a pure result computation and a separate action because that single call inherently performs the item's only effect and returns the item's result. The callback itself is that owner operation or directly returns only one call to it; it does not mutate an outer value or perform a second effect.
