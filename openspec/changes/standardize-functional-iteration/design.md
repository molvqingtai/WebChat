...
The Artico send path exposed a separate owner-approved correction during this cleanup. `Room.send` already accepts `target?: string | string[]`, while WebChat expanded an omitted target into a join-to-leave `readyPeers` membership and sent once per peer. This correction removed that adapter iteration and restored one optional-target provider delegation. The later `restore-targeted-room-sends` authority preserves that one-call adapter boundary but supersedes product target selection: every current logically room-wide producer now resolves one application-owned peer-id array before delegation.
...
- Delegate each Artico send once with its original optional target capability.
- Resolve ordinary Chat, normal Session/World publications, and one History request to explicit application-owned peer-id arrays; keep Session/World current-state catch-up and History responses targeted.
- Derive a direct join-followup Session or World target array only after its call-site-local one-second sleep, using membership current when the wait ends.
...
- No WebChat-owned per-target failure isolation. The later targeted-send authority preserves Artico's original selected-array first-throw interruption and existing whole-send failure owner.
...
- No new test case, parameter row, assertion, fixture, mock capability, test helper abstraction, or coverage requirement for the functional-iteration migration. The later targeted-send repair separately owns mutation controls for its changed targeting and join-followup timing.
...
`RoomTransport.send(roomId, payload, to?: string | string[])` keeps its optional target type. After confirming the room exists, the adapter calls `room.send(payload, to)` exactly once. It does not create a target set, read or filter `readyPeers` or DataChannel state, or loop over recipients.

Target meaning is closed:

- `to === undefined` remains one native room broadcast at the private provider-capability layer. None of the five current room-wide product producers uses this form after `restore-targeted-room-sends`.
...
- `to: string[]` addresses one already selected subset without changing the meaning to broadcast. Current room-wide product producers resolve de-duplicated, self-excluding logical recipients from their current Session/room-membership owner before delegation, without a separate grace/departure classifier.
- `to: []` is an explicit no-op and never falls back to broadcast.

The adapter does not manufacture or filter product recipients. Session, History, World, and current-generation membership owners resolve the five current product arrays. In particular, the History requester uses its existing request-start `expectedProviders` both as the loading-settlement snapshot and as the explicit target array for every inventory page.

When a Session or World publication is the direct continuation of this client's accepted join, the owning call chain performs `sleep(1000)` before it derives or filters peer ids. Only after the sleep and current-owner re-check does the product owner read current membership, de-duplicate, exclude self, and make its one array call. Membership changes during the sleep therefore affect the eventual array exactly as represented by that owner. This is not an adapter gate, separate grace/departure classifier, room-wide readiness flag, shared queue, or delay on later/user sends.

One History synchronization allocates one request identity and sends each paginated inventory-request page exactly once to its one explicit `expectedProviders` array. Every peer in that request-start Session snapshot may provide a targeted response. Response page order and bounds are validated independently by `(syncId, sourcePeerId)`, while completion, failure, and departure settle that provider only for loading. One provider's invalid page, failure, or departure must not cancel another provider or erase otherwise valid History received later. Every response page associated with the known request identity and accepted by the existing pagination validation is retained and merged through the existing message-identity deduplication regardless of arrival time, loading visibility, provider connectivity, or room generation changes. `syncId` and `sourcePeerId` only correlate and validate pages; elapsed time and generation changes do not discard an otherwise valid page.
...
The adapter adds no send-failure handling. The later targeted-send authority preserves Artico's current array behavior: a selected target throw rejects the single call with its original Error and may interrupt later targets. The source keeps `@rtco/client` and its lock resolution unchanged and adds no attempt-all, provider-call retry, outbox, acknowledgement, or error-text classifier; only the existing World release preflight retry that made zero provider calls remains.
...
Except for the target delegation, later explicit product-target classification, bounded multi-provider History settlement, direct-join timing, and exact six-file 30-day candidate-window replacement defined by current authority, every rewrite must preserve evaluation order, iteration order, call count, synchronous or asynchronous settlement, concurrency, return values, thrown and rejected errors, object identity, mutation visibility, event order, timers, storage and database operations, network/wire behavior, DOM behavior, and generated output. A shorter expression is acceptable only when it is behavior-equivalent.

Existing tests, fixtures, and harnesses may receive only the minimum behavior-equivalent iteration or private-state ownership edits necessary to satisfy the same authored-source rule, plus minimum expectation synchronization for this cleanup's transport and History corrections. An existing private fixture owner may replace its complete internal state once when that removes repeated external mutation, but its exposed object and behavior must remain equivalent. Test names, scenarios, inputs, timing, mocks, public fixture contracts, and coverage remain unchanged for this cleanup; the later targeted-send authority independently owns its focused mutation controls.
...
- **The adapter manufactures or iterates recipients again** -> Keep one `room.send(payload, to)` delegation; require product owners to supply the already selected array.
- **A selected target failure becomes WebChat attempt-all** -> Preserve Artico's original first-throw interruption and existing Error owner with no retry, outbox, acknowledgement, or classifier.
- **A join-followup snapshots recipients before waiting** -> Sleep first, re-check the exact owner, then resolve the current logical array so membership changes during the wait are retained.
- **An array-target History request changes provider settlement** -> Keep the request-start expected-provider snapshot, ten-second loading race, independent response lanes, and late valid merge unchanged.
...
3. Produce one WebChat source child that carries the reviewed four-artifact authority set and applies the functional-iteration standard, direct optional-target delegation, explicit five-producer logical-target classification, native selected-array first-error settlement, bounded multi-provider History synchronization, exact six-file 30-day replacement, generated exclusion, and no-new-test policy without adding an enforcement layer. The later targeted-send child separately owns its focused controls and sleep-then-current-target join continuations. Update only task checkboxes whose full rows are true on that exact.
