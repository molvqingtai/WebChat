...
- Bind the exact production extension worker and packaged manifest before issuing exactly one planned `Target.createTarget({ url: 'https://example.com/' })` call.
- Record one ordered timeline for worker readiness, target and main-frame navigation, the page-bound extension isolated context, normalized console/exception evidence, and final extension-shadow DOM state.
...
- No transfer of QA #287's Firefox, Chrome, static, action, Runtime, cleanup, Reviewer, or QA verdicts to a later exact.

## Acceptance Criteria

- This docs authority is a clean detached/ref-free sole child of `1b1f6cc61d7de9adc75bca0cc1b3768d90555e04`; its implementation is a clean sole child and changes only the two new diagnostic files plus checkbox-only task progress.
...
- Before the accepted page exists, the diagnostic binds the exact production package, extension ID, Service Worker, and packaged manifest, and establishes the complete observable CDP event surface.
...
- Focused controls prove that late listener installation, startup accepted-page reuse, missing/foreign worker or manifest, target replacement, context from the wrong target/frame/extension, Runtime-unavailable evidence, false host-page `#root`, missing mount, event overflow, and any retry/repair fail closed.
- Fresh QA starts from zero. Chrome proves the new lifecycle sequence, a clean mounted content Runtime, one real native action, `afterOptionsCount - beforeOptionsCount = 1`, and post-action Runtime on the same bound target, plus zero residual cleanup. Firefox reruns initial startup and two same-profile owned-process restarts; no QA #287 result transfers.
