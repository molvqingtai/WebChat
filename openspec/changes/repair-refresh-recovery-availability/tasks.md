...
- [ ] 2.5 On retry success, reuse normal join completion for retained input, message reload, finished status, and one self-join fact; on failure, reuse join failure, normalize the same request error, and return to retryable initial status.
...
- [ ] 4.2 Preserve and extend joined reconnect, duplicate request, request-local Toast settlement, stale callback, closed-panel, and unrelated Toast regression coverage.
...
- [ ] 5.3 Synchronize the accepted exact without touching `.pnpm-store/`; Owner locally verifies that Refresh is clickable after a real failed join, retry starts immediately, pending prevents duplicate clicks, and a successful retry joins chat.
