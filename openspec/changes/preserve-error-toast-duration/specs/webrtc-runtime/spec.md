...
Failure SHALL still update the matching request/readiness loading entry to the error descriptor selected by its current owning route. A genuine current-page initialization failure governed by caught-error observability SHALL use exactly the original `error.message` without prefix, suffix, wrapper, mapping, normalization, or replacement copy. An initialization failure with no current affected page/live route or no user impact SHALL call `console.error(error)` directly and SHALL NOT manufacture a Toast destination. Other operation-specific copy, including the existing ready-context `Connection failed`, SHALL remain governed by its own authority. Request settlement MAY independently clear pending/button state but SHALL NOT clear a published error presentation. Successful retry/reconnect and genuine ready transitions SHALL retain their existing matching-loading dismissal and no-success-Toast behavior. Delayed cleanup from an older request SHALL NOT dismiss a newer loading or error descriptor that uses the same stable ID. All cleanup SHALL remain ID-scoped and preserve unrelated Toasts.
...
- **THEN** WebChat SHALL update the loading entry to the error descriptor selected by the current operation's owning route, SHALL NOT actively dismiss that error, SHALL clear request/button pending state independently, and SHALL leave the error in the mounted Toast store for its ordinary lifetime
