...
The header SHALL omit the `New version` eyebrow and SHALL center the local WebChat logo, product heading, installed version, and optional release date as one compact vertical release stamp. The logo SHALL sit above the product heading, followed by a centered wrapping metadata row. The primary semantic heading SHALL identify `WebChat v<installed version>`. The page SHALL show the release date when available, the current update notes with semantic headings/lists/inline code/links, and explicit links derived from package metadata: repository `homepage`, exact `<homepage>/releases/tag/v<installed version>` Release, and `<bugs.url>/new` issue feedback. External destinations SHALL open only after activation, in a separate tab with no opener relationship. The existing Options version control SHALL open the internal page, and a manually opened rendered page SHALL acknowledge the installed version.

The surface SHALL use the local WebChat logo and the existing shadcn `Badge` and `Button` primitives without modifying or copying those primitives. It SHALL not load or render a ShieldCN or other remote badge. Its one unframed `max-w-3xl` reading column SHALL contain a compact centered release-stamp header, the natural-height current-note body beginning within 32 CSS pixels of its metadata row, and a final responsive action group. The header SHALL have no eyebrow, bottom border, rule, or divider. The visible version SHALL use a local secondary Badge, the optional date SHALL use a local outline Badge, and their combined semantic heading name SHALL identify `WebChat v<installed version>`. A missing date SHALL reserve no empty space.
...
- **THEN** the centered logo, product heading, version/date Badges, and first note heading SHALL form one compact top sequence with at most 32 CSS pixels between the centered metadata row and note content
- **AND** no `New version` eyebrow, header divider, ShieldCN or other remote badge, vertical release spine, timeline dot, forced empty band, page-level card, or bare-link footer SHALL remain
...
- **THEN** the centered logo, semantic `WebChat v<installed version>` heading, local version/date Badges, current notes, and three outbound Buttons SHALL remain legible, ordered, nonoverlapping, and fully operable
