...
- Replace the rejected release-spine presentation with one compact, unframed release record: a centered vertical logo/title/version/date header with no eyebrow or divider, notes beginning immediately below it, and a responsive action group built from the project's existing local shadcn `Badge` and `Button` primitives. Keep natural document scrolling; do not use ShieldCN or other remote badges; remove the decorative timeline, large dead space, and bare-link footer.
