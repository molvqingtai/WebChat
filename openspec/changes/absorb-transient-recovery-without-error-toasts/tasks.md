...
- [x] 2.4 Make a proven terminal native endpoint failure stop its owning generation's retry/watchdog/loading cycle and enter the existing error pass-through exactly once, without terminalizing unknown errors by default.
- [x] 2.5 Add focused regressions proving `Extension context invalidated.` is unchanged in one error toast, the failed generation stops retrying, later loading cannot replace the error, and valid recoverable transients remain silent.
