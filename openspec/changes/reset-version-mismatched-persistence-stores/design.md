...
1. The canonical message store is one IndexedDB database per page origin. Its stable logical name is `${STORAGE_NAME}:EVENTS_V2_CANONICAL_RECORDS:${origin}`, and its current private native version is `MESSAGE_STORE_VERSION = 2`.
...
The version mechanism itself is introduced as a non-destructive baseline. A missing database or missing completion value is not an old numbered generation: the implementation records or creates the current target without deleting pre-version data. From then on, every non-equal existing value resets that physical store directly to the target. Cleanup failures are observable only in the developer console.

## Goals / Non-Goals

**Goals:**

- Name the two target authorities symmetrically as `MESSAGE_STORE_VERSION` and `CONFIG_STORE_VERSION`.
...
- Preserve existing data when establishing the first version baseline and on every same-version reopen.
...
- Migrating, exporting, backing up, selectively retaining, or restoring data from a mismatched generation.
...
### 1. Two symmetric constants own two independent generations

The target constants SHALL be named at the same abstraction level:
...
Both names use `<domain>_STORE_VERSION`. `MESSAGE_STORE_VERSION` owns only the canonical message database. `CONFIG_STORE_VERSION` owns only configuration data behind the existing `Storage` boundary: per-origin `AppStatus` and extension-wide `UserInfo`. `USER_STORE_VERSION` is rejected because it would omit `AppStatus`; generic `STORAGE_VERSION` is rejected because it is not symmetric with the specific message-store owner; `APP_VERSION` and `PACKAGE_VERSION` are rejected because they reintroduce release-version coupling.

Both configurable versions and the private configuration completion key SHALL be declared in `src/constants/storage.ts`, not inside an IndexedDB, storage-adapter, background, or content implementation. Their persistence owners import them from that constants module. This follows the Owner's project rule that configurable constants live under `src/constants/`; it does not move non-configurable module-private implementation values out of their owning modules. Emoji constant organization is a separate requirement and PR.
...
| -------------------------------------------------------------- | ------------------------------------------------------------------------------------------------- |
| Physical message database absent                               | Create the canonical database directly at `MESSAGE_STORE_VERSION`; perform no delete              |
| Configuration completion value absent                          | Preserve existing values and write `CONFIG_STORE_VERSION`; perform no clear                       |
| Persisted value strictly equals its target                     | Open or load unchanged and preserve every owned value                                             |
| Persisted value exists and is not strictly equal to its target | Delete or clear the entire owned physical scope, rebuild it at the target, then record completion |

The unequal case includes adjacent upgrades, skipped upgrades, rollbacks, future values, malformed values, and any other non-equal value. Versions are generation identifiers, not an ordered migration sequence. A V1-to-V3 reset and a V3-to-V1 reset follow the same lifecycle. No smaller-than comparison, downgrade exception, per-version upgrade callback, shape detection, compatibility decode, or best-effort conversion is allowed.

The missing-value rule deliberately preserves installations that predate the version markers. It also makes a clean install non-destructive. Absence cannot distinguish those cases, and the Owner chose the current release as the baseline rather than retrospectively clearing pre-version data.

### 3. Physical scope is exact and reset is complete

For a message mismatch, the implementation deletes exactly the current origin's canonical database `${STORAGE_NAME}:EVENTS_V2_CANONICAL_RECORDS:${origin}`. Deleting the database removes every old object store, row, index, canonical `records` value, bounded `conflicts` diagnostic, and unknown residue inside that identity. It then recreates the same logical name from the target private `MessageDatabaseSchema` at `MESSAGE_STORE_VERSION`. Clearing known stores in place is insufficient because a skipped generation can contain unknown schema residue; renaming the database would abandon the stable canonical identity.
...
The background cannot enumerate or access every page origin's local namespace or canonical message database. The content injection for an origin therefore SHALL, before constructing the Remesh store or mounting WebChat:
...
3. await the current origin's canonical message-database check and any target recreation.

Origins are handled lazily when they are next injected; an update SHALL not enumerate tabs merely to delete data. No affected application read, watch, write, history query, default-state write, or UI mount may start before all three required checks settle ready. The two store families MAY prepare independently, and a successfully completed family is not rolled back or repeated merely because the other one failed.

### 5. Reset ownership is serialized per physical identity

The decision and reset form one exclusive lifecycle keyed by physical store identity and target version. Concurrent callers for the same identity SHALL join the current lifecycle or acquire ownership only after it finishes and re-read persisted state. A caller that observes target completion SHALL open or load the target without issuing another delete or clear. This prevents a late same-version contender from erasing new-generation messages, identity, preferences, or status written after the first reset.
...
If message deletion is blocked, the current application remains unmounted and the request SHALL NOT be reported as ready. The implementation MAY log one bounded console diagnostic and await the same native request; it SHALL not display a close-tabs warning, start a second delete, time out into a late unowned deletion, or open the old database. When the blocker releases, the same lifecycle may complete. If its execution context ends, the next injection re-observes the physical database and starts or joins the required lifecycle from persisted state.

### 6. Completion is durable; failure is console-only and retryable

The irreversible message boundary is native database deletion success. If execution stops before it, the old database remains non-current and the next injection retries. If execution stops after deletion but before target recreation finishes, the next injection observes absence, creates an empty target, and does not restore old values. Native target creation at `MESSAGE_STORE_VERSION` is the message completion state; there is no second message migration marker.
...
1. Freeze this requirements-only OpenSpec authority on one requirement branch and PR without changing source or advancing either store version.
2. After explicit Owner/Planner release, implement the two private reset lifecycles and their deterministic regressions as the sole source continuation on the same requirement branch and PR.
...
None. The Owner has fixed the two persistence families, symmetric constant names, strict non-equality semantics, baseline, independent scope, lazy origin handling, direct reset, awaited completion, console-only failure, and ordinary app-version exclusion.
