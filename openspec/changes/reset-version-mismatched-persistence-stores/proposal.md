...
- **BREAKING (local data)**: Replace the active per-origin database identity with direct `STORAGE_NAME`, whose exact current value is `WEB_CHAT_STORAGE`. IndexedDB's separate API namespace and origin partition prevent collision with the `${STORAGE_NAME}:` localStorage prefix. The released-retired `${STORAGE_NAME}:EVENTS_V2_CANONICAL_RECORDS:${origin}` database and every other non-target database are not looked up, opened, read, migrated, converted, exported, cleared, or deleted. An absent target is created empty, while an already-existing same-version target is preserved byte-for-byte.
...
- Keep every non-target canonical database and deprecated unstorage message data outside every active lifecycle: they remain untouched and provide no data, readiness, failure, or cleanup signal. Keep host-page storage outside the WebChat namespace, unrelated IndexedDB databases, other origins not currently injected, and browser storage areas outside the configuration-store ownership untouched.
...
- Unchanged surfaces: every non-target and deprecated unstorage message database; public `Database`, `MessageStore`, and `Storage` interfaces; Chat/World and peer wire; Runtime lifecycle ownership; Memory database semantics; and dependencies.
