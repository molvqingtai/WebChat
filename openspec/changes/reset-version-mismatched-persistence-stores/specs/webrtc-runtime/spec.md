...
Every non-target database SHALL remain unread, unconverted, uncleared, unmarked, and outside readiness. The active unstorage-backed `LocalStorageImpl` and `BrowserSyncStorageImpl` SHALL remain configuration scopes and reset only under `CONFIG_STORE_VERSION`. The public `Database`, `MessageStore`, and `Storage` contracts, Memory database behavior, Runtime ownership, Chat/World behavior, peer wire, and dependency graph SHALL remain unchanged.
