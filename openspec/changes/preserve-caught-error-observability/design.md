...
| Unit and E2E code                   | No product Toast is created by a test harness.                                                                                                                      | Unit tests assert the expected error; E2E cleanup attaches ordered evidence rather than relying on console output alone.                                                               |
...
The five unit-test sinks must retain task handles and assert their exact expected rejection/terminal reason. Recovery's additional no-op `onError(() => {})` listener is outside the 36 catch/rejection-handler count but must also be replaced by captured and asserted errors. The E2E close controls must prove cleanup evidence and primary-result preservation. Existing source search and review must account for all 36 sites; this change adds no parser, linter, fixture-only production branch, or dependency.
