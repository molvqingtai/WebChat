...
- Make request and response pages continuous, bounded, replay-safe, and current-session-only. Disconnect, timeout, invalid order, local processing failure, or transport departure discards the attempt; reconnect starts a fresh `syncId` and recomputes the exact difference.
...
- Affected verification: strict wire rejection, inventory/difference pagination, fixed snapshots, replay/order/caps, reconnect recomputation, concurrent insertion races, per-sync Toast ownership, completion/cancellation dismissal, and deletion of the old full-window path.
