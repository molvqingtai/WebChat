...
- [x] 4.2 Replace History runtime tests with both directional flows, fixed 30-day snapshots, exact filtering, empty phases, ordering/replay/caps, supplier cancellation, local send semantics, response serialization, exactly one synchronization per connection/direction, terminal same/different-ID rejection, domain-release fence cleanup, and an independent next-connection synchronization with no continued progress.
