...
- [x] 1.3 Use current v5 Chat and World room namespaces, exclude `session-end`, and prove v1-v5 isolation with no fallback or dual publication.

## 2. Replace History Orchestration End To End

- [x] 2.1 Delete the cursor/full-window requester/provider state machine and make each accepted room connection trigger exactly one outgoing requester synchronization with a fixed 30-day ID snapshot, independent `syncId`, and paged inventory output. The later triggering-source target repair is tracked by `scope-history-sync-to-trigger-peer`.
- [x] 2.2 Bind the first valid incoming page zero as that connection's sole provider `syncId`, wait for the complete inventory, freeze one 30-day record snapshot, filter the exact ID set, sort the complete snapshot oldest-first, and stream the resulting missing-record pages with exact per-page authors.
- [x] 2.3 Enforce phase-zero start, continuous pages, explicit empty completion, active identical-replay idempotency, changed replay/gap/post-done rejection, no cumulative entry/byte phase budget, a fixed 10-second operational timeout whose accepted progress does not re-arm or replace the timer, source-local cancellation, and a constant-size terminal ID fence that rejects both same- and different-ID restarts.
...
- [x] 4.1 Replace protocol fixtures and tests with exact current structures, declarative limits, opaque-ID frame bounds, old-shape and `session-end` rejection, v5 isolation, retained payload/codec controls, and no callback-backed author-reference rejection.
- [x] 4.2 Replace History runtime tests with both directional flows, fixed 30-day snapshots, exact filtering, provider oldest-first pagination, empty phases, replay/page bounds, supplier cancellation, local send semantics, response serialization, exactly one synchronization per connection/direction, terminal same/different-ID rejection, domain-release fence cleanup, and an independent next-connection synchronization with no continued progress. The later source-only target and no-established-pair-restart controls are tracked by `scope-history-sync-to-trigger-peer`.
