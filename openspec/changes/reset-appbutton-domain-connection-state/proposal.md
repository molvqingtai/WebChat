...
- Modify the existing v6 SESSION lifecycle requirement across reconnect, automatic recovery, page attach/reattach, grace return, and supported host recovery: replace its absolute post-expiry non-resurrection rule so an observer-stale `ended` record cannot reject a strictly matching logical presence when a current trusted source proves a new physical session binding. Superseded room/source work, the ended physical `sessionId`, identity/time mutation, and truly stale replay remain rejected.
...
- Keep the cumulative v6 peer wire messages, schemas, codec limits, room identifiers, and public `ChatRoom` interface unchanged; add no compatibility path.
