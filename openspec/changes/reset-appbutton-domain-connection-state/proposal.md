...
- Correct the shared SESSION lifecycle rule across reconnect, automatic recovery, page attach/reattach, grace return, and supported host recovery: an observer-stale `ended` record cannot reject a strictly matching logical presence when a current trusted source proves a new physical session binding. Superseded room/source work, the ended physical `sessionId`, identity/time mutation, and truly stale replay remain rejected.
