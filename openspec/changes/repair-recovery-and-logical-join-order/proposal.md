...
- Retain the `repair-refresh-recovery-availability` contract and source exact `a6021495` as inherited history, while binding resumed source work to current authority exact `3a0a7d1fa11b1f69ce73f93a55f05e8e88cc48b1` and a new docs child on the existing `fix/cross-browser-action-lifecycle` / Draft PR #76 route. The invalid `9beec650...` line and its evidence are not implementation inputs.
...
- `webrtc-runtime`: Define disconnected-peer reactivation, internal supersession cancellation, logical join-order classification, fragment-insensitive page routing/bootstrap, bounded ClientLease recovery/readiness feedback, silent Refresh success, and the combined verification/release boundary.
