...
The inherited baseline is source exact `a602149522c7038f29e13307bb925a48ed3848d7`, whose parent docs exact `740373298711002548cbe6eecb3c63dcb045db74` already restores Refresh after terminal join failure. Current authority exact `9552156151aabaabdece5a7c3c77d732c07eb356` on `fix/cross-browser-action-lifecycle` / Draft PR #76 preserves the non-doc source bytes of `cd1f054301abb383100b89fdea5bcb633ac79332` and the seven-repair product contract. Its focused/full automated evidence is exact-local and remains useful history, but the Owner has deferred partial manual acceptance and now requires one final candidate containing seven repairs. The Owner removed the successful manual-Refresh `Ready to chat` Toast while explicitly retaining visible `Connecting` for every active Refresh or real connect/join/recovery operation and excluding passive polling; request-local loading, Refresh control motion, and genuine failure feedback remain.
...
- Show owner-scoped `Connecting` throughout every current Refresh or actual connect/join/recovery operation, including healthy retained-Runtime reattachment, while ensuring it dismisses on current ready or becomes unavailable within the bounded recovery budget and never becomes `Ready to chat`; passive polling or a health probe that has not promoted into such an operation shows none and leaves Refresh unchanged.
...
Passive polling remains outside that presentation owner. A periodic health check, provider identity probe, or watchdog observation that has not yet promoted into an actual connect, join, register/attach recovery, or host rebuild SHALL publish no `Connecting` entry and SHALL not disable or rotate Refresh. A healthy result leaves presentation unchanged. If the observation proves that repair is required, one loading owner begins exactly when the lifecycle promotes into the actual recovery operation; it inherits the existing lifecycle's remaining deadline rather than resetting it. Poll completion itself cannot dismiss, replace, or report the terminal result of that operation.
...
- passive healthy/error polling with no presentation change; polling promotion into actual recovery without deadline reset; retained healthy host refresh, hung/rejected register or attach, host/Port/response loss, concurrent recovery signals, late completion, and current recovery success/failure crossed with owner-scoped Connecting from operation promotion plus bounded HostPhase and feedback settlement.
...
1. Freeze this refinement as a clean detached sole child of current authority exact `9552156151aabaabdece5a7c3c77d732c07eb356`; keep the Owner checkout, `.pnpm-store/`, unrelated refs, PR body/state, and CI untouched. Continue the same `fix/cross-browser-action-lifecycle` / Draft PR #76 requirement route; do not create another branch/PR or restore/cherry-pick invalid `9beec650...` bytes or evidence.
2. Implement v3 protocol/Session logical time and namespace isolation first, then Artico liveness, typed supersession, canonical fragment-insensitive page routing, deadline-owned ClientLease recovery/readiness feedback, Tabs API-owned physical tab lifecycle, and cumulative Refresh integration on one controlled route. The current Owner-smoke and tab-lifecycle gaps require fail-first coverage before their repairs.
