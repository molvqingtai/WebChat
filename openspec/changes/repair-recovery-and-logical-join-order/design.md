...
The inherited baseline is source exact `a602149522c7038f29e13307bb925a48ed3848d7`, whose parent docs exact `740373298711002548cbe6eecb3c63dcb045db74` already restores Refresh after terminal join failure. The current valid source exact is `cd1f054301abb383100b89fdea5bcb633ac79332` on `fix/cross-browser-action-lifecycle` / Draft PR #76. Its focused/full automated evidence is exact-local and remains useful history, but the Owner has deferred partial manual acceptance and requires one final candidate containing all six repairs. The Owner removed the successful manual-Refresh `Ready to chat` Toast while explicitly retaining visible `Connecting` for every active Refresh/recovery; request-local loading and genuine failure feedback remain.
...
Every repaired path needs a deterministic fail-before that fails for the documented parent cause and a pass-after on the same final candidate. Protocol tests SHALL cover exact keys, missing/unknown/invalid `joinedAt`, generation binding, canonical bytes, and namespace residue. Provider tests SHALL prove exactly one replacement peer and stale callback/timer isolation. ClientLease tests SHALL use deterministic time and controllable pending RPCs to prove per-call and overall deadlines, cancellation, single-flight, late-response fencing, immediate Connecting even for healthy retained-host refresh, and bounded terminal feedback. Application tests SHALL include two explicit parent fail-first controls: successful Refresh currently publishes `Ready to chat` instead of ID-scoped dismissal, and both-late discovery plus historical SESSION currently persists a pre-join peer notice. Candidate controls SHALL prove active Connecting remains visible, success dismisses only its owner without a success descriptor, genuine error remains, and exact A-before-B notice projections are A `[A joined, B joined]` and B `[B joined]`. Existing a602 Refresh controls remain mandatory but their prior verdicts do not transfer.
...
1. Freeze this authority as a clean detached sole child of current valid source exact `cd1f054301abb383100b89fdea5bcb633ac79332`; keep the Owner checkout, `.pnpm-store/`, remote refs, PR body/state, and CI untouched. Continue the same `fix/cross-browser-action-lifecycle` / Draft PR #76 requirement route; do not create another branch/PR or restore/cherry-pick invalid `9beec650...` bytes or evidence.
...
5. Only after both pass, synchronize the Owner checkout and run one six-scenario manual acceptance: failed-join Refresh with active Connecting followed by no success Toast, disconnected-peer retry, multi-page avatar update without supersession Toast, A-before-B exact notice order, direct startup on a fragment URL with a visible working control, and retained-Runtime page refresh with active then terminal bounded Connecting.
