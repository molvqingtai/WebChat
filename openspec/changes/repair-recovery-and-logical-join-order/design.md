...
The inherited baseline is source exact `a602149522c7038f29e13307bb925a48ed3848d7`, whose parent docs exact `740373298711002548cbe6eecb3c63dcb045db74` already restores Refresh after terminal join failure. Current authority exact `cd01035768b05d9708bd6781f016e8b9742086e6` on `fix/cross-browser-action-lifecycle` / Draft PR #76 preserves the non-doc source bytes of `cd1f054301abb383100b89fdea5bcb633ac79332` and the prior six-repair product contract. Its focused/full automated evidence is exact-local and remains useful history, but the Owner has deferred partial manual acceptance and now requires one final candidate containing seven repairs. The Owner removed the successful manual-Refresh `Ready to chat` Toast while explicitly retaining visible `Connecting` for every active Refresh/recovery; request-local loading, Refresh control motion, and genuine failure feedback remain.
...
Watchdog, ping, heartbeat, and Port observations SHALL remain connectivity inputs only. Their loss MAY start or join the current bounded recovery generation, but SHALL NOT remove the background's physical tab ownership, decrement the authoritative tab registry, start last-tab grace, retire logical membership, or publish SESSION_END.
...
1. Freeze this refinement as a clean detached sole child of current authority exact `cd01035768b05d9708bd6781f016e8b9742086e6`; keep the Owner checkout, `.pnpm-store/`, unrelated refs, PR body/state, and CI untouched. Continue the same `fix/cross-browser-action-lifecycle` / Draft PR #76 requirement route; do not create another branch/PR or restore/cherry-pick invalid `9beec650...` bytes or evidence.
