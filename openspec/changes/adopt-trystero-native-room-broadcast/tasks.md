...
- [x] 1.5 Obtain fresh independent docs review on one immutable authority exact.

## 2. Inventory And Fail-Before Evidence

- [x] 2.1 Inventory every production Room send and classify it as native room-wide broadcast or one of the retained History/Session/World targeted sends.
- [x] 2.2 Inventory all fixed post-join sleeps, 999/1000ms controls, peer-id filtering/recomputation, special room-wide resume types/events/commands, and generic queue/fence behavior that must remain.
- [x] 2.3 Inventory all Artico source, test, dependency/lock, README/current-documentation, composition, and structural-rule residue plus the current RoomTransport file layout.
- [x] 2.4 Add or identify mutation-sensitive fail-before controls for broadcast omission, retained explicit targets, late-active-peer catch-up, zero active peers, generic queue fencing, provider layout, and Artico residue.
