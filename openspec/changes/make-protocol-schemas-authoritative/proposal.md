...
- Validate protocol values through the same pure static schema at exactly three boundaries: accepting a decoded peer message, local send once before either persistence or codec encoding, and loading a message from local persistence. A failed receive/load parse is discarded before application or projection and produces no Toast; a failed local-send parse persists and sends nothing.
- Remove protocol validation from local production before the send boundary, persistence write and encoding after it, History supply, and intermediate Runtime paths while preserving non-protocol ownership and lifecycle decisions.
...
- `webrtc-runtime`: Limit protocol validation to peer receive, local send, and local persistence load through the same pure static schema; replace final-end lifecycle ownership with PeerLeave-owned observer grace.
...
- Affected consumers: inbound Wire parsing, local send before persistence/encoding, local `MessageStore` reads, duplicate protocol checks, Session physical-leave classification, Connection release, and obsolete final-end storage/send/retry paths.
- Affected tests: protocol schema/type authority, declarative-only residue, receive/send/load rejection, absence of unsupported validation, Push/Pull naming, v5 isolation, strict `session-end` rejection, PeerLeave grace, same-presence recovery, multi-presence expiry, and silent parse-failure coverage.
