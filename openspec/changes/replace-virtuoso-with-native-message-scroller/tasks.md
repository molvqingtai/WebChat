...
- [x] 1.2 Keep initial latest-message presentation, bottom-only smooth follow, off-bottom count plus click recovery, manual departure, and history-prepend anchor preservation as real-DOM behaviors.
...
- [x] 2.2 Rebuild MessageList on the Message Scroller primitives with the follow/unread layer limited to chat semantics the primitive does not carry; keep the provider/viewport/content shell constantly mounted across loading/empty/loaded phases and gate only rows on non-null content.
