...
- [x] 1.2 Keep initial latest-message presentation, bottom-only smooth follow as one current follow authorization (transient self-caused off-bottom geometry neither counts nor cancels it; strictly advanced geometry/tail retargets; `scrollend` settles with one deduped reconciliation command; real input intent cancels), off-bottom count plus click recovery, manual departure, and history-prepend anchor preservation as real-DOM behaviors.
