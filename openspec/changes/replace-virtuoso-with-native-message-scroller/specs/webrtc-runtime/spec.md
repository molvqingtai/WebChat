...
The message list SHALL smooth-follow a new tail only while settled at the bottom, as one current follow authorization: while the authorization is unsettled, transient off-bottom geometry caused by its own motion or by an intrinsic reserve converting to a taller real row SHALL neither increment the arrival count nor cancel it; a newer tail commit or native scroll event SHALL retarget only when scroll height or tail generation strictly advanced; native `scrollend` SHALL settle it, retiring at physical bottom or issuing one deduped smooth command to the current max when geometry advanced; real wheel, touch, navigation-key, and scrollbar-drag intent SHALL cancel it, while pointer, click, selection, and programmatic scroll events SHALL NOT. The list SHALL count off-bottom arrivals behind one click-recovery action, respect manual departure, and preserve the reading anchor across history prepends. The recovery action SHALL appear only while the exact bottom distance is greater than 0.5× the current viewport client height and SHALL stay hidden at or below that distance even while arrivals are counted, with the count retained internally; that distance SHALL be recomputed on scroll and on shell or content resize. Exactly one app-side ResizeObserver MAY observe the message viewport and content to keep that geometry current and to reconcile a strict post-settle scroll-height growth: only while the settled position physically remains at the previous bottom and no follow authorization owns retargeting SHALL it issue one deduplicated auto scroll-to-end command for a new height or tail generation; manual off-bottom reading SHALL never be reclaimed, later tails SHALL count normally, and the observer callback SHALL NOT self-loop or duplicate the same generation. No timer, debounce, polling, retry loop, or animation-frame loop SHALL be used for this behavior. The footer autocomplete SHALL keep ArrowUp/ArrowDown/Enter selection and focus semantics and keep the active option natively visible. Production, tests, configuration, package manifest, lockfile, and current documentation SHALL contain no `react-virtuoso` dependency, identity, type, mock, or Virtuoso-only mechanism; archived historical change records MAY retain Virtuoso names for their own immutable exacts.
...
#### Scenario: Distance-gated recovery visibility

- **WHEN** the exact bottom distance is at or below 0.5× the current viewport client height, including while arrivals are being counted
- **THEN** the recovery action SHALL remain hidden with the count retained internally, SHALL appear only beyond that distance, and SHALL track the distance across scroll and shell or content resize

#### Scenario: Post-settle growth reconciliation

- **WHEN** content grows after the list has settled at the bottom, or after a manual off-bottom departure
- **THEN** the settled-at-bottom growth SHALL be reconciled once to the new bottom while bottom-follow continuity is intact, and the manual off-bottom position SHALL never be reclaimed, with later tails counting normally
