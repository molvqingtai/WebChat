...
- `MAX_CHAT_EVENT_BYTES` is `192KiB` as both the static declarative Text body ceiling and the sole local user Text capacity preflight. Before dispatch, Footer computes `getTextByteSize(JSON.stringify({ body, mentions }))`; an oversized value keeps the draft and shows `Message size cannot exceed 192KiB.` without entering Schema parsing, wire, or persistence.
...
- Existing affected tests, fixtures, and active OpenSpec state reflect these values and the sole Footer preflight. The contract adds no test case, test abstraction, compatibility behavior, or additional resource guard.
...
- Affected source: protocol limit constants and declarative consumers, the sole local user Text Footer preflight, and History session-limit state and branches.
- Affected verification: exact-value, codec-limit, authoring-size, and History no-cumulative-limit expectations only.
- Unchanged: editor image representation and lifecycle, schemas and protocol structures, v5 namespaces, History messages/phases/pagination, 30-day snapshots, timeout ownership, persistence, delivery, identity, dependencies, public APIs, and every unlisted resource boundary.
