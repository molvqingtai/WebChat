...
- `MAX_CHAT_EVENT_BYTES` is `192KiB` at its existing consumers and enforcement points.
...
- Existing affected tests, fixtures, user-facing size copy, and active OpenSpec state reflect these values. The contract adds no test case, test abstraction, compatibility behavior, or additional resource guard.
...
- Affected source: protocol limit constants and consumers, History session-limit state and branches, and the footer size copy.
